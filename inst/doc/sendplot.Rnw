...
Currently we recommend using mozilla firefox web browser since Internet Explorer does not meet requirements for viewing. Compatiblity issues will be resolved in future versions. This current version only runs in linux. Platform compatibility will also be resolved in future versions.\\

\quad We will now go through an example of running this application. This example is a more advanced usage; for basic examples please see the R help files for sendplot, sendxy or sendimage. We will go through the steps to make Figure 1 where the main image heatmap is interactive.
...
sendplot <- function(mat, plot.calls, mai.mat, x,y, xlim, ylim,
                     ...
                     bound.pt = TRUE,resize="4000x5500", ps.paper="letter",
...
@ 
\quad The sendplot arguments ps.paper, ps.width, and ps.height are passed into the postscript call for paper, width, and height. A postscript, static image, is generated automatically when the sendplot function is called. The resize argument is passed as part of a system convert command to convert the postscript to the .png. The original 800 by 1100 image is resized to this dimension. This helps expand condensed looking images, or visa versa. For our example plot we wish to resize making the final image smaller in both width and height. 
...
sendplot(mat=mat, plot.calls=plot.calls, mai.mat=mai.mat,
         ...
         resize=resize,bound.pt=TRUE, kolourpaint=TRUE)
\end{verbatim}
...
sendplot(mat=mat, plot.calls=plot.calls, mai.mat=mai.mat,
         ...
         resize=resize, bound.pt=FALSE, kolourpaint=FALSE)

@ 
The resulting html file may be openned in any web browser that is capable of running javascript. We recommend using mozilla firefox; Internet Explorer currently is not supported. Briefly, the spot.radius argument controls how large an area will be active when the mouse is scrolled over. If the user selects a larger region, some spot locations may overlap and be lost. If the user selects a low region, the interactive application is very sensitive. 
