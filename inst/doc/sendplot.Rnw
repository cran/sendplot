...
\indent{\bf{Important Note:}} The sendplot library requires two system requirements: libtiff and ImageMagick.  The libtiff application is utilized by the required dependent package rtiff. This is used for comparing tiff images. The libtiff application is free for download at www.libtiff.org. The application ImagicMagick is utilized by sendplot to convert between file types. It is freely available for download at www.imagemagick.org. Users may need system administrative privledges to install new applications on their computer; If this is the case, see your own IT department for assistance in installing. 
