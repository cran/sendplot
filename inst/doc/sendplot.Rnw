...
\indent {\bf{Note:}} In layout, zero acts as a region that no graph is displayed, a buffer. Notice the use of zero to allow the first and fourth plot to line up in the example.
...
\indent {\bf{Note:}} If figure margins are too large, an error will occur when plotting.  If the user gets an Error figure margins too large, try decreasing the values in mai.mat.
