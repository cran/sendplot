...
#
# maps x and y values as x and y
#  checks and alters lbls/links matricies
#

makeScatterDF <- function(Splot,
                          xlim, ylim,
                          x.pos,y.pos,
                          boundingPt,
                          x.labels=NA,
  ...
  # dimension check
  if(contx){
    if((dim(x.labels)[1] != length(x.pos)) & contx){
      contx = FALSE
      cat(paste("Warning: x.labels does not have correct dimensions \n   number of rows should equal length(x.pos):",length(x.pos), "\n   Continuing with x.labels = NA \n", sep=""))
      x.labels = NA
    }
  }
  ...
  # dimension check
  if((dim(y.labels)[1] != length(y.pos)) & conty){
    conty = FALSE
    cat(paste("Warning: y.labels does not have correct dimensions \n   number of rows should equal length(y.pos):",length(y.pos), "\n   Continuing with y.labels = NA \n", sep=""))
    y.labels = NA
  }      
  ...
  # dimension check
  if(((dim(xy.labels)[1] != length(y.pos)) | (dim(xy.labels)[1] != length(x.pos))) & contxy){
    contxy = FALSE
    cat(paste("Warning: xy.labels does not have correct dimensions \n   number of rows should equal length(y.pos):",length(y.pos), " or length(x.pos):", length(x.pos), "\n   Continuing with xy.labels = NA \n", sep=""))
    xy.labels = NA
  }         
  ...
  # dimension check
  if(((dim(xy.links)[1] != length(y.pos)) | (dim(xy.links)[1] != length(x.pos))) & cont){
    cont = FALSE
  ...
  if((length(asLinks) == 1) & !is.na(asLinks[1])) asLinks = rep(asLinks, length(x.pos))
  # convert to character vector
  asLinks = as.character(asLinks)
  # check dimensions
  if((length(asLinks) != length(x.pos)) & !is.na(asLinks[1])){
    cat("Warning: cannot create points as links \n     length must be equal to x.pos or y.pos \n")
    contLinks = FALSE
  }
  if(length(asLinks) ==1){
  ...
  MapObj$Pixcoord = paste(as.character(x.new), as.character(y.new), sep=",")
  MapObj$dat = dat
  MapObj$dat2 = dat2
  MapObj$dat3 = dat3
