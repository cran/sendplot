...
sendimage <- function(plot.call,
  ...
  sendplot(mat = mat, plot.calls = plot.calls, mai.mat = mai.mat , type="image",x = x, y = y,z=z, z.value=z.value, plt.extras=plt.extras, x.lbls = x.lbls,y.lbls = y.lbls,xy.lbls = xy.lbls,bound.pt=bound.pt,resize=resize, ps.paper=ps.paper,ps.width=ps.width,ps.height=ps.height,fname.root=fname.root,dir=dir, kolourpaint=kolourpaint,up.left=up.left,low.right=low.right,spot.radius=spot.radius)
